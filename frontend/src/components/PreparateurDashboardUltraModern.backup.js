...
const PreparateurDashboardUltraModern = ({ user }) => {
  ...
  const calculateUrgency = useCallback((dossier) => {
  ...
  const fetchDossiers = useCallback(async () => {
    ...
    try {
      ...
      // 🔍 DIAGNOSTIC: Exposer les dossiers pour analyse
      if (process.env.NODE_ENV === 'development') {
        window.preparateurDossiers = dossiersList;
        console.log('📊 [PreparateurDashboard] Dossiers chargés:', dossiersList.length);
        
        // Vérifier les IDs
        const withoutId = dossiersList.filter(d => !d.id);
        if (withoutId.length > 0) {
          console.warn(`⚠️ ${withoutId.length} dossiers sans ID:`, withoutId);
        }
      }
      
    } catch (error) {
      // Gestion d'erreur : récupération des dossiers échouée
  ...
  const handleViewDetails = (dossier) => {
    // 🔍 DIAGNOSTIC: Log le dossier cliqué
    if (process.env.NODE_ENV === 'development') {
      console.log('👁️ [handleViewDetails] Dossier cliqué:', {
        numero: dossier.displayNumber,
        client: dossier.displayClient,
        id: dossier.id,
        folder_id: dossier.folder_id,
        dossier_id: dossier.dossier_id,
        numero_dossier: dossier.numero_dossier,
        all_fields: Object.keys(dossier)
      });
    }
    ...
    if (!dossierId || dossierId === 'null' || dossierId === 'undefined' || dossierId === null || dossierId === undefined) {
      console.error('❌ [handleViewDetails] ID invalide pour dossier:', {
        dossier: dossier,
        displayNumber: dossier.displayNumber,
        displayClient: dossier.displayClient,
        tentative_id: dossierId
      });
      notificationService.error(`Impossible d'ouvrir le dossier "${dossier.displayNumber}" : ID invalide`);
      return;
    }
    
    console.log('✅ [handleViewDetails] ID valide, ouverture modal avec ID:', dossierId);
    setSelectedDossier(dossier);
    setShowDetailsModal(true);
  };

  // Logique de tri
  const sortDossiers = useCallback((dossiersList) => {
  ...
  }, [dossiers, searchTerm, filterStatus, filterType, filterPriority, sortDossiers]);
      ...
      <div className="p-4 space-y-4 max-h-[600px] overflow-y-auto custom-scrollbar">
        {dossiers.length === 0 ? (
          <div className="text-center py-12 text-neutral-400 dark:text-neutral-500">
            <Icon className="h-16 w-16 mx-auto mb-3 text-neutral-300 dark:text-neutral-600" />
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-100 via-purple-50 to-pink-100 p-6">
          ...
          <div className="flex flex-col lg:flex-row gap-6 items-start lg:items-center">
            ...
            <div className="flex-1 relative">
              <motion.div 
                ...
                className="relative"
              >
                <MagnifyingGlassIcon className="h-6 w-6 text-neutral-400 dark:text-neutral-500 absolute left-4 top-1/2 transform -translate-y-1/2" />
                <input
                  type="text"
      ...
      {/* Modal création dossier - utilisation directe du composant */}
      <CreateDossier
